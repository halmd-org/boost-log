...
basic_core< CharT >::add_global_attribute(attribute_name_type const& name, attribute_factory const& attr)
{
    BOOST_LOG_EXPR_IF_MT(typename implementation::scoped_write_lock lock(pImpl->Mutex);)
...
template< typename CharT >
std::pair< typename basic_core< CharT >::attribute_set_type::iterator, bool >
basic_core< CharT >::add_thread_attribute(attribute_name_type const& name, attribute_factory const& attr)
{
