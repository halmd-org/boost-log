...
template< typename CharT >
BOOST_LOG_EXPORT unlocked_frontend< CharT >::unlocked_frontend(shared_ptr< void > const& backend, consume_trampoline_t consume_tramp) :
...
template< typename CharT >
BOOST_LOG_EXPORT synchronous_frontend< CharT >::synchronous_frontend(shared_ptr< void > const& backend, consume_trampoline_t consume_tramp) :
    base_type(new implementation(backend, consume_tramp))
{
    BOOST_ASSERT(!!backend);
}
